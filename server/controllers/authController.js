...
export const register = async (req, res, next) => {
  try {
    ...
    // Create user
    // ⚠️ Безопасность: не выдаём роль admin только по совпадению email.
    // Нормальный режим: admin = (ADMIN_EMAIL совпал) + (правильный ADMIN_INVITE_CODE).
    // Для совместимости: если ADMIN_INVITE_CODE НЕ задан, то поведение остаётся как раньше,
    // но выводим предупреждение в консоль.
    let isAdmin = false;
    if (process.env.ADMIN_EMAIL && email === process.env.ADMIN_EMAIL) {
      if (process.env.ADMIN_INVITE_CODE) {
        isAdmin = adminCode === process.env.ADMIN_INVITE_CODE;
      } else {
        isAdmin = true;
        console.warn('⚠️ ADMIN_INVITE_CODE не задан. Admin будет выдан только по ADMIN_EMAIL (небезопасно).');
      }
    }
